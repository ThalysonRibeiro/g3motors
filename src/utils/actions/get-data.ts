...
export async function getDataHome() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/objects/67879ef252686af6a3aa326a?pretty=true&read_key=${process.env.READ_KEY}&depth=1&props=slug,title,metadata,type`, { next: { revalidate: 120 } });
...
export async function getItemBySlug(itemSlug: string) {
  ...
  // definido o objeto de consulta pelo slug
